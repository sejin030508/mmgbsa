...
# 에러 발생 시 즉시 종료 설정 (단, || 연산자로 예외 처리된 부분은 스크립트를 종료시키지 않음)
set -e
...
for SYSTEM_DIR in "$DATA_BASE_DIR"/*/; do
    ...
    mkdir -p "$WORK_DIR" || { echo "🚨 [$PDB_ID] 디렉터리 생성 실패. 다음 타겟으로 건너뜁니다."; continue; }
    
    # 원본 파일 복사
    cp "$SYSTEM_DIR/${PDB_ID}_protein.pdb" "$WORK_DIR/" || { echo "🚨 [$PDB_ID] 단백질 PDB 파일 복사 실패. 건너뜁니다."; continue; }
    cp "$SYSTEM_DIR/${PDB_ID}_ligand_opt.mol2" "$WORK_DIR/" || { echo "🚨 [$PDB_ID] 리간드 Mol2 파일 복사 실패. 건너뜁니다."; continue; }
    
    cd "$WORK_DIR" || { echo "🚨 [$PDB_ID] 작업 폴더 이동 실패. 건너뜁니다."; continue; }
    ...
    pdb4amber -i ${PDB_ID}_protein.pdb -o protein_noh.pdb --nohyd > pdb4amber.log 2>&1 || { echo "🚨 [$PDB_ID] pdb4amber 실행 실패. 로그를 확인하세요. 건너뜁니다."; continue; }
    
    # 2. 리간드 파라미터화 (Antechamber)
    echo "[$PDB_ID] 2/6 리간드 파라미터화 중..."
    antechamber -i ${PDB_ID}_ligand_opt.mol2 -fi mol2 -o ligand_bcc.mol2 -fo mol2 -c bcc -s 2 > antechamber.log 2>&1 || { echo "🚨 [$PDB_ID] Antechamber 실행 실패. 건너뜁니다."; continue; }
    parmchk2 -i ligand_bcc.mol2 -f mol2 -o ligand.frcmod > parmchk2.log 2>&1 || { echo "🚨 [$PDB_ID] parmchk2 실행 실패. 건너뜁니다."; continue; }
...
quit
EOF
    tleap -f tleap.in > tleap.log 2>&1 || { echo "🚨 [$PDB_ID] Tleap 실행 실패. 건너뜁니다."; continue; }
    ambpdb -p complex.prmtop -c complex.inpcrd > complex_reference_raw.pdb || { echo "🚨 [$PDB_ID] ambpdb 변환 실패. 건너뜁니다."; continue; }
...
with open('complex_reference_raw.pdb', 'r') as fin, open('complex_reference.pdb', 'w') as fout:
    for line in fin:
        ...
        fout.write(line)
EOF
    python fix_pdb_for_bk.py || { echo "🚨 [$PDB_ID] Python PDB 패치 스크립트 실행 실패. 건너뜁니다."; continue; }
    ...
    cd "$BIOKINEMA_DIR" || { echo "🚨 BioKinema 디렉터리 이동 실패. 건너뜁니다."; continue; }
    ...
    bash inference.sh \
        ...
        --checkpoint_path ./biokinema.pt > "$WORK_DIR/biokinema.log" 2>&1 || { echo "🚨 [$PDB_ID] BioKinema AI 궤적 추론 실패. 건너뜁니다."; continue; }
    
    cd "$WORK_DIR" || { echo "🚨 작업 폴더 복귀 실패. 건너뜁니다."; continue; }
    ...
    python "$SCRIPT_DIR/npy_to_dcd.py" "$WORK_DIR" > npy_to_dcd.log 2>&1 || { echo "🚨 [$PDB_ID] DCD 변환(npy_to_dcd.py) 실패. 건너뜁니다."; continue; }
    ...
    MMPBSA.py -O -i "$SCRIPT_DIR/mmpbsa.in" \
        ...
        -rp receptor.prmtop \
        -lp ligand.prmtop \
        -y biokinema_trajectory.dcd > mmpbsa.log 2>&1 || { echo "🚨 [$PDB_ID] MMPBSA 결합 에너지 계산 실패. 건너뜁니다."; continue; }
    ...
    if [ -f "FINAL_RESULTS_MMPBSA.dat" ]; then
        ...
        echo "🚨 [$PDB_ID] 최종 결과 파일(FINAL_RESULTS_MMPBSA.dat)이 생성되지 않았습니다."
    fi
done
