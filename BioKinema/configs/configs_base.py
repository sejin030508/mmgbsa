...
model_configs = {
    ...
    "model": {
        ...
        "N_cycle": 4,
        # When True, ignore the precomputed s_inputs/s/z in input_feature_dict and compute
        # them fresh via input_embedder + msa_module + pairformer_stack each step, with
        # gradients enabled (lets the embedder + pairformer fine-tune). Costs more memory &
        # compute. Default False = use precomputed embeddings (see scripts/encode_embeddings.sh).
        "train_input_embedder": False,
        "input_embedder": {
        ...
        "diffusion_module": {
            ...
            # Temporal-bias exponent β in B_ij = -λ|t_i-t_j|^β. Released: 0.5 (sqrt), 0.25 (long MD).
            "beta": 1.0,
...
loss_configs = {
    "loss": {
        ...
        "weight": {
            ...
            "alpha_bond": 1.0,  # or 1 in finetuning stages
            "alpha_lig_bond": 1.0, 
            "alpha_center": 0.0, 
            ...
            "alpha_velocity": 2.0,
            "alpha_rel_rmsf": 8.0,
            "alpha_local_rmsf": 8.0,
            "alpha_acf": 0.0,  # Cα lag-1 ACF loss weight; effective = alpha_diffusion * alpha_acf * alpha_ensemble
            "alpha_tica_dynamics": 0.1,  # TICA dynamics loss weight (transition + population + autocorrelation); gated by has_msm
            "max_acf_lag": 5,  # max lag for autocorrelation loss
        },
        # When True, MSELoss aligns GT to pred (original AF3 Algorithm 23 behavior).
        "mse_align": False,
        "plddt": {
            "min_bin": 0,
            "max_bin": 1.0,
        ...
        "tica_dynamics": {
            "temperature": 2.0,  # soft state assignment temperature
            "weight_transition": 0.5,  # transition matching sub-weight
            "weight_population": 1.0,  # population matching sub-weight
            "weight_autocorrelation": 0.0,  # autocorrelation matching sub-weight
        },
    },
    "metrics": {
        "lddt": {
