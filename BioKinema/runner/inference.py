...
class InferenceRunner(object):
    ...
    def predict(self, data: Mapping[str, Mapping[str, Any]]) -> dict[str, torch.Tensor]:
        ...
        data["input_feature_dict"]["frame0_coordinate"] = data["label_dict"]["coordinate_0"]
        with enable_amp:
...
import gzip
import pickle
def get_bioassembly_dict(file_path):
    # Define the path to your .pkl.gz file
    try:
        # Open the gzipped pickle file in binary read mode
...
def infer_predict(runner: InferenceRunner, configs: Any) -> None:
    ...
    _, dataloader_all = get_dataloaders(configs=configs,
                                    world_size=1,
                                    seed=42,
                                    error_dir=runner.error_dir,)
    
    dump_root = runner.dump_dir

    for dataloader in list(dataloader_all.values()):
        ...
        for seed in configs.seeds:
            ...
            for sample_index, batch in enumerate(dataloader):
                try:
                    data = batch
                    atom_array = batch["cropped_atom_array"]
                    basic = batch["basic"]
    
                    # bioassembly_dict = get_bioassembly_dict(basic["bioassembly_dict_fpath"])
                    ...
                    sample_name = basic["pdb_id"]
                    dump_dir = runner.dumper._get_dump_dir("", sample_name, seed)
                    if os.path.exists(dump_dir):
                        print("already exists:", dump_dir)
                        continue
                    logger.info(
                        (
                            f"[Rank {DIST_WRAPPER.rank} ({sample_index + 1}/{num_data})] {sample_name}: "
                            f"N_asym {basic['N_asym'].item()}, N_token {basic['N_token'].item()}, "
                            f"N_atom {basic['N_atom'].item()}, N_msa {basic['N_msa'].item()}"
                        )
                    )
                    new_configs = update_inference_configs(configs, basic["N_token"].item())
                    runner.update_model_configs(new_configs)
                    prediction = runner.predict(data)
                    os.makedirs(dump_dir, exist_ok=True)
                    np.save(os.path.join(dump_dir, f"{sample_name}_pred_coordinates.npy"), prediction["coordinate"].detach().cpu().numpy())
                    np.save(os.path.join(dump_dir, f"{sample_name}_is_ligand.npy"), data["input_feature_dict"]["is_ligand"].detach().cpu().numpy())
                    np.save(os.path.join(dump_dir, f"{sample_name}_atom_to_tokatom_idx.npy"), data["input_feature_dict"]["atom_to_tokatom_idx"].detach().cpu().numpy())
                    np.save(os.path.join(dump_dir, f"{sample_name}_atom_to_token_idx.npy"), data["input_feature_dict"]["atom_to_token_idx"].detach().cpu().numpy())
                    np.save(os.path.join(dump_dir, f"{sample_name}_coordinate_mask.npy"), data["input_feature_dict"]["frame0_coordinate_mask"].detach().cpu().numpy())
    
                    if "label_dict" in data:
                        ...
                        np.save(os.path.join(dump_dir, f"{sample_name}_GT_coordinates.npy"), x_original.detach().cpu().numpy())
                    ...
                    runner.dumper.dump(
                        ...
                        entity_poly_type=None, #bioassembly_dict["entity_poly_type"],
                    )
    
                    logger.info(
                        f"[Rank {DIST_WRAPPER.rank}] {sample_name} succeeded.\n"
                        f"Results saved to {configs.dump_dir}"
                    )
                    torch.cuda.empty_cache()
                except Exception as e:
                    error_message = f"[Rank {DIST_WRAPPER.rank}]{sample_name} {e}:\n{traceback.format_exc()}"
