...
class AF3Trainer(object):
    ...
    def try_load_checkpoint(self):
        ...
        def _load_checkpoint(
            ...
            if (not sample_key.startswith("module.")) and self.use_ddp:
                checkpoint["model"] = {
    ...
    def model_forward(self, batch: dict, mode: str = "train") -> tuple[dict, dict]:
        assert mode in ["train", "eval", "encode"]
        if mode == "encode":
            log_dict = None
            with torch.no_grad():
                if not self.use_ddp:
                    batch["s_inputs"], batch["s"], batch["z"]  = self.model.get_pairformer_output(
                    ...
                    batch["s_inputs"], batch["s"], batch["z"]  = self.model.module.get_pairformer_output(
                        input_feature_dict=batch["input_feature_dict"],
                        N_cycle=self.model.module.N_cycle,
                        inplace_safe=True,
                    )
        else:
    ...
    def _evaluate(self, ema_suffix: str = "", mode: str = "eval"):
        ...
        for test_name, test_dl in self.test_dls.items():
            ...
            for index, batch in enumerate(tqdm(test_dl)):
                if "traj_len" not in batch["label_dict"].keys():
                    print(f"traj_len not found in {index}, {batch['basic']['traj_name']}")
                    continue
                ...
                with enable_amp:
                    try:
                        ...
                        self.print(f"Rank {DIST_WRAPPER.rank}: OOM error, skip {pid}")
    ...
    def train_step(self, batch: dict):
        ...
        # One-shot verification (only when train_input_embedder=true): on the very
        # first backward, report gradient norms grouped by top-level submodule so
        # we can confirm input_embedder / msa_module / pairformer_stack are actually
        # receiving gradients (i.e., not frozen).
        if (
            self.global_step == 0
            and self.configs.model.get("train_input_embedder", False)
            and not getattr(self, "_grad_diag_printed", False)
        ):
            self._grad_diag_printed = True
            from collections import defaultdict
            grad_groups = defaultdict(lambda: [0.0, 0, 0])  # [sum_norm, n_params, n_with_grad]
            for name, p in self.model.named_parameters():
                bare = name[len("module."):] if name.startswith("module.") else name
                group = bare.split(".")[0]
                grad_groups[group][1] += 1
                if p.grad is not None:
                    grad_groups[group][0] += float(p.grad.detach().float().norm())
                    grad_groups[group][2] += 1
            self.print(f"[grad-diag] per-submodule gradient norms after first backward:")
            for g in sorted(grad_groups):
                gn, n, ng = grad_groups[g]
                self.print(f"  {g:<35} grad_norm_sum={gn:.4e}  ({ng}/{n} params with grad)")
    ...
    def run(self):
        ...
        while True:
            for batch in self.train_dl:
                ...
                batch = to_device(batch, self.device)
                # Update train step in dataset for pseudo-trajectory scheduling
                if hasattr(self.train_dl, 'dataset'):
                    ds = self.train_dl.dataset
                    if hasattr(ds, 'set_train_step'):
                        ds.set_train_step(self.step)
                    elif hasattr(ds, 'datasets'):
                        # WeightedMultiDataset: propagate to all sub-datasets
                        for sub_ds in ds.datasets:
                            if hasattr(sub_ds, 'set_train_step'):
                                sub_ds.set_train_step(self.step)
                self.progress_bar()
    ...
    def dump_embeddings(self):
        # load test set and dump embeddings
        # Init Metric Aggregator
        simple_metric_wrapper = SimpleMetricAggregator(["avg"])
        eval_precision = {
            "fp32": torch.float32,
        ...
        # dump_dir = "/cto_studio/xtalpi_lab/fengbin/Protenix_v0.2.0/Protenix/atlas/pairformer_emb/dump"
        ...
        # else:
        #     dump_dir = "/cto_studio/xtalpi_lab/fengbin/datas/embed_mdposit_nomsa"

        # dump_dir = "/cto_studio/xtalpi_lab/fengbin/Protenix_v0.2.0/Protenix/fb_analysis/completed_embed_files"
        # dump_dir = "/cto_studio/xtalpi_lab/fengbin/Protenix_v0.2.0/Protenix/fb_analysis/completed_embed_files_nomsa"
        # dump_dir = "/cto_studio/xtalpi_lab/fengbin/Protenix_v0.2.0/Protenix/atlas/pairformer_emb/dump_nomsa"
        # dump_dir = "/cto_studio/xtalpi_lab/fengbin/datas/bioemu/embed_files"
        # dump_dir = "/cto_studio/xtalpi_lab/fengbin/datas/embed_unbinding_nomsa"
        # dump_dir = "/cto_studio/xtalpi_lab/fengbin/datas/embed_misato_msa_noref"
        data_config = self.configs.data

        for test_name, test_dl in self.test_dls.items():
            config_dict = data_config[test_name].to_dict()
            dump_dir = config_dict["precomputed_emb_dir"]
            self.print(f"Testing on {test_name}")
            ...
            for index, batch in enumerate(tqdm(test_dl, desc="iterating target embeddings")):
                pid = batch["basic"]["pdb_id"]
                # traj_idx = int(pid.split("_")[-1])
                if "unbinding" in test_name:
                    traj_name = batch["basic"]["pdb_id"].split('_Pro_lig')[0]
                elif "misato" in test_name:
                    traj_name = batch["basic"]["pdb_id"].split("_")[0]
                elif test_name.startswith("MSR-"):
                    traj_name = pid.split("|")[0]
                else:
                    traj_name = '_'.join(pid.split("_")[:3])
                if os.path.exists(f"{dump_dir}/{traj_name}.pt"):
                    continue

                batch = to_device(batch, self.device)
                evaluated_traj_names.append(traj_name)
                with enable_amp:
                    # Model forward
                    batch, _ = self.model_forward(batch, mode="encode")
                    output_emb = {
                        "s_inputs": batch["s_inputs"],
                        "s": batch["s"],
                        "z": batch["z"],
                    }
                    torch.save(output_emb, f"{dump_dir}/{traj_name}.pt")
                    print(f"saving to {dump_dir}/{traj_name}.pt")
...
def main():
    ...
    try:
        if configs.get("dump_embeddings", False):
            print("Dumping embeddings mode on")
            trainer.dump_embeddings()
            return
        else:
            trainer.run()
    except Exception as e:
        if not os.listdir(trainer.checkpoint_dir):
            os.system(f"rm -r {trainer.run_dir}")
        raise e
