...
class DataDumper:
    ...
    def _get_dump_dir(self, dataset_name: str, sample_name: str, seed: int) -> str:
        ...
        dump_dir = os.path.join(
            self.base_dir, dataset_name, sample_name,
        )
        return dump_dir
    ...
    def _save_structure(
        ...
        N_sample = pred_coordinates.shape[1]
        for idx in range(N_sample):
            for frame_idx in range(pred_coordinates.shape[0]):
                output_fpath = os.path.join(
                    prediction_save_dir, f"{sample_name}_s{idx}_f{frame_idx}.cif"
                )
                
                save_structure_cif(
                    atom_array=atom_array,
                    pred_coordinate=pred_coordinates[frame_idx, idx],
