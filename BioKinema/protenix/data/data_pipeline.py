...
from protenix.data.parser import DistillationMMCIFParser, MMCIFParser, ATLASMMCIFParser, MisatoMMCIFParser, UnbindingMMCIFParser, MDpositMMCIFParser, InferenceMMCIFParser
    ...
    def get_data_from_mmcif(
        ...
        **kwargs,
        ...
        try:
            ...
            elif "atlas" in dataset.lower():
                parser = ATLASMMCIFParser(mmcif_file=mmcif)
                bioassembly_dict = parser.get_structure_dict(**kwargs)
            elif "unbinding" in dataset.lower():
                parser = UnbindingMMCIFParser(mmcif_file=mmcif)
                bioassembly_dict = parser.get_structure_dict(**kwargs)
            elif "misato" in dataset.lower():
                parser = MisatoMMCIFParser(mmcif_file=mmcif)
                bioassembly_dict = parser.get_structure_dict(**kwargs)
            elif "mdposit" in dataset.lower():
                parser = MDpositMMCIFParser(mmcif_file=mmcif)
                bioassembly_dict = parser.get_structure_dict(**kwargs)
            elif "inference" in dataset.lower():
                parser = InferenceMMCIFParser(mmcif_file=mmcif)
                bioassembly_dict = parser.get_structure_dict(**kwargs)
            else:
                raise NotImplementedError(
                    'Unsupported "dataset", please input either "WeightedPDB" or "Distillation" or "Atlas".'
                )
