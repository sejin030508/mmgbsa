...
class MMCIFParser:
    ...
    def __init__(self, mmcif_file: Union[str, Path]):
        ...
        self._mmcif_file = Path(mmcif_file)
    ...
    def get_structure(
        ...
        # FBTODO, add occupancy annotations here
        ...
        # Ligand (res_name 'LIG') bond handling: drop LIG-internal residue-name bonds,
        # then add distance-based intra-ligand bonds.
        # 获取蛋白质中残基名为LIG的原子
        ligand_name = "LIG"
        lig_mask = atoms.res_name == ligand_name

        lig_global_indices = np.where(lig_mask)[0]

        # =========================================================================
        # [修改点]：在提取SDF键之前，先清除 bonds 中属于 LIG 内部的旧键
        # =========================================================================
            
        # 获取当前所有的 bond 数组 (通常是 N x 2 或 N x 3 的 numpy array)
        current_bond_array = bonds.as_array()
            
        if len(current_bond_array) > 0:
            # 检查 bond 的两个原子索引是否都在 lig_global_indices 中
            # np.isin 返回一个布尔数组，指示元素是否在测试元素集中
            mask_atom1 = np.isin(current_bond_array[:, 0], lig_global_indices)
            mask_atom2 = np.isin(current_bond_array[:, 1], lig_global_indices)
                
            # 如果两个原子都在 LIG 中，则该 bond 是 LIG 内部的 bond，需要被标记为 True
            internal_bond_mask = mask_atom1 & mask_atom2
                
            # 如果存在内部键，则进行过滤
            if np.any(internal_bond_mask):
                # 保留 (~internal_bond_mask) 为 True 的行，即保留 "不完全在LIG内部" 的键
                kept_bonds = current_bond_array[~internal_bond_mask]
                    
                # 使用过滤后的数组重新构建 BondList 对象
                # 注意：这里假设 struc 是 biotite.structure 或兼容库
                bonds = struc.BondList(atoms.array_length(), kept_bonds)


        # add dist bond for ligands
        bonds_dist = struc.connect_via_distances(atoms, inter_residue=False)

        # 1. 获取键两端原子的残基名
        bond_indices = bonds_dist.as_array()[:, :2]
        res_names_1 = atoms.res_name[bond_indices[:, 0]]
        res_names_2 = atoms.res_name[bond_indices[:, 1]]
            
        # 2. 创建掩码，筛选配体键 (这里配体名叫 'MK1')
            
        # ligand_name = "___"
        mask = (res_names_1 == ligand_name) & (res_names_2 == ligand_name)
            
        # 3. 应用掩码
        ligand_bond_indices = bond_indices[mask]
        # print(f"步骤3: 从 'bonds_dist' 中筛选出 {len(ligand_bond_indices)} 个属于 '{ligand_name}' 的键。")
            
        # 4. 创建配体键的 BondList
        ligand_bonds = struc.BondList(atoms.array_length(), bonds=ligand_bond_indices)
            
        # 5. 合并
        bonds = bonds.merge(ligand_bonds)
    ...
    def make_indices(
        ...
        meta_dict = {
            ...
            "frame_id": bioassembly_dict["frame_id"],
...
def get_res3_sequence(atom_array):
    res3_seq = atom_array[atom_array.atom_name == 'CA'].res_name
    res1 = []
    for res in res3_seq:
        if res in mmcif_restype_3to1:
            res1.append(mmcif_restype_3to1[res])
        else:
            res1.append('X')
    return ''.join(res1)


class ATLASMMCIFParser(MMCIFParser):
    def get_structure_dict(self) -> dict[str, Any]:
        """
        Get an AtomArray from a CIF file of distillation data.
        ...
        # patches
        # auth_asym_id
        parent_name = self._mmcif_file.parent.name
        chain_id = self._mmcif_file.stem.split("_")[1]
        atom_array.chain_id[:] = chain_id
        atom_array = atom_array[atom_array.element != "H"]
        atom_array.set_annotation("element", [x.upper() for x in atom_array.element])

        self.entity_poly_type = {'1': 'polypeptide(L)'}

        structure_dict = {
            "pdb_id": parent_name + "|" + self._mmcif_file.stem,
            "traj_name": parent_name + "|" + '_'.join(self._mmcif_file.stem.split('_')[:-1]), # pdb_chain_R_idx
            "frame_id": int(self._mmcif_file.stem.split("_")[-1]),
            "atom_array": None,
            "assembly_id": None,
            "sequences": {'1': get_res3_sequence(atom_array)},
            "entity_poly_type": self.entity_poly_type,
            "num_tokens": -1,
            "num_prot_chains": -1,
        }
        ...
        for func in pipeline_functions:
            ...
            if len(atom_array) == 0:
                # no atoms left
                print("warning: no atoms left")
                return structure_dict
        
        # FBTODO, add is_resolved annotation based on occupancy here
        ...
        # the number of protein chains in the structure
        structure_dict["num_prot_chains"] = 1
        structure_dict["atom_array"] = atom_array
        structure_dict["num_tokens"] = atom_array.centre_atom_mask.sum()
        return structure_dict
    ...
    def add_missing_atoms_and_residues(self, atom_array: AtomArray) -> AtomArray:
        ...
        if "occupancy" not in atom_array.get_annotation_categories():
            ...
            occupancy = atom_array.get_annotation("occupancy")
            # add is_resolved annotation
            is_resolved = np.zeros(len(atom_array), dtype=bool)
            is_resolved[occupancy == 1.0] = True
            if (occupancy == 0.0).sum() / len(occupancy) > 0.20:
                print(f"Warning: more than 20% atoms are missing in the cif {self._mmcif_file}")
            atom_array.set_annotation("is_resolved", is_resolved)
            return atom_array


class OpenmmMMCIFParser(MMCIFParser):
    def get_structure_dict(self, **annotations) -> dict[str, Any]:
        """
        Get an AtomArray from a CIF file of distillation data.

        annotations: dict contains energy values as structure-level labels
        ...
        structure_dict = {
            "pdb_id": self._mmcif_file.stem, # pdb_chain_frame
            "traj_name": '_'.join(self._mmcif_file.stem.split('_')[:3]), # pdb_chain_R_idx
            "frame_id": int(self._mmcif_file.stem.split("_")[-1]),
            "atom_array": None,
            "assembly_id": None,
            "sequences": {'1': get_res3_sequence(atom_array)},
        ...
        if annotations is not None and len(annotations) > 0:
            structure_dict[f"openmm_annotation"] = annotations
...
class UnbindingMMCIFParser(MMCIFParser):
    def get_structure_dict(self, output_dir=None) -> dict[str, Any]:
        """
        Get an AtomArray from a CIF file of distillation data.
        ...
        """
        # created AtomArray of first model from mmcif atom_site (Asymmetric Unit)
        pdb_id= self._mmcif_file.parts[-3]
        traj_id=self._mmcif_file.parts[-2]
        if traj_id == "temp_fixed":
            pdb_id = self._mmcif_file.parts[-4]
            traj_id=self._mmcif_file.parts[-3]
        traj_name = f"{pdb_id}_{traj_id}"
        ...
        structure_dict = {
            "pdb_id": f"{traj_name}_{frame_id}",
            "traj_name": f"{traj_name}", # pdb_chain_R_idx
            "frame_id": frame_id,
            "atom_array": None,
            "assembly_id": None,
            # "sequences": self.get_sequences(atom_array),
        ...
        atom_array = AddAtomArrayAnnot.add_token_mol_type_byname(atom_array)
        atom_array = AddAtomArrayAnnot.add_centre_atom_mask(atom_array)
        atom_array = AddAtomArrayAnnot.add_atom_mol_type_mask(atom_array)
        atom_array = AddAtomArrayAnnot.add_distogram_rep_atom_mask(atom_array)
        atom_array = AddAtomArrayAnnot.add_plddt_m_rep_atom_mask(atom_array)
        ...
        self.entity_poly_type = self.get_entity_poly_type_from_atoms(atom_array)
        structure_dict["entity_poly_type"] = self.entity_poly_type
        structure_dict["sequences"] = self.get_sequences(atom_array)

        # rename chain_ids from A A B to A0 A1 B0 and add asym_id_int, entity_id_int, sym_id_int 
        atom_array = AddAtomArrayAnnot.unique_chain_and_add_ids(atom_array)
        atom_array = AddAtomArrayAnnot.find_equiv_mol_and_assign_ids(
            atom_array, self.entity_poly_type, by_entity=True
        )

        # numerical encoding of (chain id, residue index)
        atom_array = AddAtomArrayAnnot.add_ref_space_uid(atom_array)

        atom_array = AddAtomArrayAnnot.add_ref_info_and_res_perm(atom_array, copy_mask_from_pos=True)
    ...
    def get_sequences(self, atom_array: AtomArray) -> dict[str, str]:
        starts = struc.get_chain_starts(atom_array, add_exclusive_stop=True)
        sequences = {}
        ...
        for start, stop in zip(starts[:-1], starts[1:]):
            entity_id = atom_array.label_entity_id[start]
            seq = get_res3_sequence(atom_array[start:stop])

            # skip if centre atoms within a chain are all unresolved, e.g. 1zc8
            if ~np.any(is_centre_atom_and_is_resolved[start:stop]):
                continue
            if "polypeptide" in self.entity_poly_type[entity_id]:
                sequences[entity_id] = seq
                
        return sequences
    
    def get_entity_poly_type_from_atoms(self, atom_array: AtomArray) -> dict[str, str]:
        chain_indices = self.make_chain_indices(atom_array)
        entity_poly_type = {}
        for chain_dict in chain_indices:
            entity_id = chain_dict["entity_id"]
            chain_id = chain_dict["chain_id"]
            mol_type = chain_dict["mol_type"]
            if mol_type.lower() == "prot":
                entity_poly_type[entity_id] = "polypeptide(L)"
            else:
                entity_poly_type[entity_id] = "other"
    ...
    def make_chain_indices(
        ...
        for start, stop in zip(starts[:-1], starts[1:]):
            chain_id = atom_array.chain_id[start]
            entity_id = atom_array.label_entity_id[start]
            mol_type = atom_array[start].mol_type
...
class MisatoMMCIFParser(MMCIFParser):
    def get_structure_dict(self, output_dir=None) -> dict[str, Any]:
        """
        Get an AtomArray from a CIF file of distillation data.
        ...
        """
        # created AtomArray of first model from mmcif atom_site (Asymmetric Unit)
        pdb_id= self._mmcif_file.parts[-2]
        if pdb_id == "temp_fixed":
            pdb_id = self._mmcif_file.parts[-3]

        atom_array = self.get_structure(pdb_id=pdb_id)
        
        atom_array = atom_array[atom_array.element != "H"]
        atom_array.set_annotation("element", [x.upper() for x in atom_array.element])
        

        # traj_id=self._mmcif_file.parts[-2].split("lig")[-1]
        frame_id=int(self._mmcif_file.stem.split("_")[-1])

        structure_dict = {
            "pdb_id": f"{pdb_id}_{frame_id}",
            "traj_name": f"{pdb_id}", # pdb_chain_R_idx
            "frame_id": frame_id,
            "atom_array": None,
            "assembly_id": None,
            # "sequences": self.get_sequences(atom_array),
        ...
        # numerical encoding of (chain id, residue index)
        atom_array = AddAtomArrayAnnot.add_ref_space_uid(atom_array)
        # TODO add ref_path argument
        # ref_file = os.path.join(output_dir,"0.pkl.gz")
        # if not os.path.exists(ref_file):
        #     print(f"Warnning: missing ref structure {ref_file}")
        #     ref_file = None

        atom_array = AddAtomArrayAnnot.add_ref_info_and_res_perm(atom_array, copy_mask_from_pos=False)
...
class MDpositMMCIFParser(MMCIFParser):
    def get_structure_dict(self, output_dir=None) -> dict[str, Any]:
        """
        Get an AtomArray from a CIF file of distillation data.
        ...
        atom_array = AddAtomArrayAnnot.add_token_mol_type_mdposit(atom_array)
        atom_array = AddAtomArrayAnnot.add_centre_atom_mask(atom_array)
        atom_array = AddAtomArrayAnnot.add_atom_mol_type_mask(atom_array)
        atom_array = AddAtomArrayAnnot.fix_ligand_bond(atom_array)
        
        # lig_global_indices = np.where(atom_array.is_ligand == 1)[0]
        # if len(lig_global_indices) > 0:
        #     residue_names = atom_array.res_name[lig_global_indices]
        #     print(f"residue_names for {pdb_id}:", set(residue_names.tolist()))
        ...
        atom_array = AddAtomArrayAnnot.add_ref_info_and_res_perm(atom_array, strict=False, copy_mask_from_pos=False)
...
class AddAtomArrayAnnot(object):
    ...
    def add_token_mol_type_byname(
        ...
        starts = struc.get_residue_starts(atom_array, add_exclusive_stop=True)
        if "LIG" not in atom_array.res_name:
            raise ValueError("not found LIG in atom_array.res_name")
    ...
    @staticmethod
    def add_token_mol_type_mdposit(
        atom_array: AtomArray
    ) -> AtomArray:
        """
        Add molecule types in atom_arry.mol_type based on ccd pdbx_type.
    ...
    @staticmethod
    def fix_ligand_bond(atoms: AtomArray) -> AtomArray:
        """
        Fix ligand bond issue.
        """
        bonds = atoms.bonds

        lig_global_indices = np.where(atoms.is_ligand == 1)[0]
        
        # residue_names = atoms.res_name[lig_global_indices]
        # print("residue_names:", set(residue_names.tolist())
        ...
        if len(current_bond_array) > 0:
            ...
            if np.any(internal_bond_mask):
                # 保留 (~internal_bond_mask) 为 True 的行，即保留 "不完全在LIG内部" 的键
                kept_bonds = current_bond_array[~internal_bond_mask]
                removed_bonds = current_bond_array[internal_bond_mask]
        ...
        # 1. 获取键两端原子的残基名
        bond_indices = bonds_dist.as_array()[:, :2]
        is_1_ligand = atoms.is_ligand[bond_indices[:, 0]]
        is_2_ligand = atoms.is_ligand[bond_indices[:, 1]]

        
        # 2. 创建掩码，筛选配体键
        
        # ligand_name = "___"
        mask = (is_1_ligand == 1) & (is_2_ligand == 1)
        
        # 3. 应用掩码
        ligand_bond_indices = bond_indices[mask]
        # print(f"步骤3: 从 'bonds_dist' 中筛选出 {len(ligand_bond_indices)} 个属于 '{ligand_name}' 的键。")
        # print("added bond indices:", ligand_bond_indices, "\n removed bond indices:", removed_bonds)
    ...
    def find_equiv_mol_and_assign_ids(
        ...
        if by_entity:
            if entity_poly_type is None:
                protein_indices = np.where(atom_array.is_protein)[0]
                ligand_indices = np.where(atom_array.is_ligand)[0]
                mol_indices = [protein_indices, ligand_indices]
                # TODO misato will recognize the protein as ligand 
                if len(ligand_indices)>100:
                    print(f"ligand atom number {len(ligand_indices)} is large than 100, carefully check if proteins are recognized as ligands")
            else:
                mol_indices = []
                for entity_id, mol_type in entity_poly_type.items():
                    entity_indices = np.where(atom_array.label_entity_id == entity_id)[0]
                    mol_indices.append(entity_indices)
    ...
    def add_tokatom_idx(atom_array: AtomArray) -> AtomArray:
        ...
        for atom in atom_array:
            ...
            if atom.mol_type == "ligand" or atom_name_position is None:
                ...
                #     print(f"atom_name: {atom.atom_name} not in {atom.res_name}")
                #     raise KeyError(f"{e}")
    ...
    def add_ref_feat_info(
        atom_array: AtomArray, ligand_ccdid: str, strict: bool = True, copy_mask_from_pos: bool = False
        ...
        for start, stop in zip(starts[:-1], starts[1:]):
            ...
            ref_result = info_dict.get(res_atom[0].ref_space_uid)
            if res_atom[0].is_ligand: # ignore reference stru for ligands
                ref_result = None

            if ref_result is not None:
                atom_map, coord, charge, mask = ref_result
                atom_names = res_atom.atom_name
                for name in atom_names:
                    if name not in atom_map:
                        if strict:
                            raise ValueError(
                                f"Atom name {name} not found in reference structure for residue {res_atom.res_name[0]}"
                            )
                        else:
                            print(
                                f"Atom name {name} not found in reference structure for residue {res_atom.res_name[0]}, ignore adding reference info"
                            )
                            ref_result = None
                            error_residue.append(res_atom[0].ref_space_uid)
                            break
            
            for atom in res_atom:
                if ref_result is None:
                    # get conformer failed
                    if copy_mask_from_pos and atom.is_ligand:
                        ref_mask.append(atom.is_ligand)
                        ref_pos.append(atom.coord)
                        ref_charge.append(0)
                    else:
                        ref_mask.append(0)
        ...
        return ref_pos, ref_charge, ref_mask, error_residue

    @staticmethod
    def add_res_perm(
        atom_array: AtomArray, ligand_ccdid: str, error_residue: list[int]
    ) -> tuple[np.ndarray, np.ndarray, list[int]]:
        """
        ...
        for start, stop in zip(starts[:-1], starts[1:]):
            ...
            if res_atom[0].ref_space_uid in set(error_residue):
                res_perm.extend([[i] for i in curr_res_atom_idx])
                continue

            ccd_id = res_atom.res_name[0]
            if res_atom.is_ligand[0]: # ignore reference stru for ligands
                ccd_id = "LIG"
            if ccd_id == "LIG":
                if ligand_ccdid is not None:
                    ccd_id = ligand_ccdid
            ...
            res_dict = get_ccd_ref_info(ccd_code=ccd_id)
            if not res_dict:
                res_perm.extend([[i] for i in curr_res_atom_idx])
                continue
    ...
    @staticmethod
    def add_ref_info_and_res_perm(atom_array: AtomArray, ligand_ccdid: str = None, strict: bool = True, copy_mask_from_pos: bool = False) -> AtomArray:
        """
        ...
        """
        ref_pos, ref_charge, ref_mask, error_residue = AddAtomArrayAnnot.add_ref_feat_info(atom_array, ligand_ccdid, strict, copy_mask_from_pos=copy_mask_from_pos)
        res_perm = AddAtomArrayAnnot.add_res_perm(atom_array, ligand_ccdid, error_residue)
...
# ============================================================================
# Inference-time parser, ported from the BioKinema inference tree. Used by
# protenix.data.data_pipeline when dataset == 'inference'.
# ============================================================================
class InferenceMMCIFParser(MMCIFParser):
    def get_structure_dict(self, output_dir=None) -> dict[str, Any]:
        """
        Get an AtomArray from a CIF file of distillation data.
        ...
        # Do not use parent folder name as pdb_id: CIFs under .../predictions/*.cif would
        # all get pdb_id "predictions_*", colliding in MSA cache and dump dirs.
        stem = self._mmcif_file.stem
        parts = self._mmcif_file.parts
        ...
        if parent.lower() == "predictions" and len(parts) >= 4:
            traj_name = parts[-4]
        else:
            traj_name = parent if parent else stem
        ...
        # Count actual protein chains. Inference inputs may be heteromeric complexes
        # (for example, two proteins bridged by a molecular glue ligand).
        prot_label_entity_ids = [
            entity_id
            for entity_id, poly_type in self.entity_poly_type.items()
            if "polypeptide" in poly_type
        ]
        structure_dict["num_prot_chains"] = len(
            np.unique(
                atom_array.chain_id[
                    np.isin(atom_array.label_entity_id, prot_label_entity_ids)
                ]
