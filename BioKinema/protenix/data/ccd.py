...
def get_component_atom_array(
    ...
    """
    if ccd_code.endswith("5") or ccd_code.endswith("3"):
        res = get_mol_type(ccd_code[:-1]) 
        if res in ["dna", "rna"]:
            return get_component_atom_array(ccd_code[:-1])
...
def get_one_letter_code(ccd_code: str) -> Union[str, None]:
    ...
    if ccd_code.endswith("5") or ccd_code.endswith("3"):
        res = get_mol_type(ccd_code[:-1]) 
        if res in ["dna", "rna"]:
            return get_one_letter_code(ccd_code[:-1])
...
def get_component_rdkit_mol(ccd_code: str) -> Union[Chem.Mol, None]:
    ...
    if ccd_code.endswith("5") or ccd_code.endswith("3"):
        res = get_mol_type(ccd_code[:-1]) 
        if res in ["dna", "rna"]:
            return get_component_rdkit_mol(ccd_code[:-1])
