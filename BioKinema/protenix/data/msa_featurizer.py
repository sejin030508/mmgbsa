...
from protenix.data.msa_cache import get_or_create_msa
...
def msa_search(
    ...
    """
    Get content-addressed MSAs for protein sequences, searching only on cache miss.
    """

    def build_one(sequence: str, build_dir: Path, query_fasta: Path) -> None:
        RequestParser.msa_search(
            seqs_pending_msa=[sequence],
            tmp_fasta_fpath=str(query_fasta),
            msa_res_dir=str(build_dir),
            mode=mode,
            raise_on_error=True,
        )
        if mode == "protenix":
            result_dirs = RequestParser.msa_postprocess(
                seqs_pending_msa=[sequence],
                msa_res_dir=str(build_dir),
            )
            if len(result_dirs) != 1:
                raise RuntimeError(
                    f"Expected one MSA result directory, got {len(result_dirs)}"
                )
...
class PROTMSAFeaturizer(BaseMSAFeaturizer):
    def __init__(
        ...
        **kwargs,
        ...
        self.msa_cache_path = msa_cache_path
    ...
    def get_msa_path(self, db_name: str, sequence: str, pdb_id: str, is_homomer_or_monomer=True) -> str:
        """
        ...
        if self.indexing_method == "pdb_id" and self.distillation_pdb_id_to_msa_dir:
            ...
            # print("self.seq_to_pdb_idx", len(self.seq_to_pdb_idx))
            if sequence in self.seq_to_pdb_idx:
                # print("bingo, sequence is in self.seq_to_pdb_idx", sequence)
                ...
                # run msa_search to get msa
                # print("sequence not in self.seq_to_pdb_idx", sequence)
                # raise ValueError(f"sequence {sequence} not in self.seq_to_pdb_idx")
                protein_seqs = [sequence]
                msa_res_subdirs = msa_search(
                    protein_seqs, self.msa_cache_path, "protenix"
                )
                # print(msa_res_subdirs[0])
                if is_homomer_or_monomer:
                    return opjoin(msa_res_subdirs[0], f"non_pairing.a3m")
                else:
                    return opjoin(msa_res_subdirs[0], f"pairing.a3m")
    ...
    def process_single_sequence(
        ...
        if not is_homomer_or_monomer:
            ...
            raw_msa_paths, seq_limits = [], []
            if opexists(
                path := self.get_msa_path(self.pairing_db, sequence, pdb_id, is_homomer_or_monomer)
            ) and path.endswith(".a3m"):
                raw_msa_paths = [
                    path,
                ]
...
def tokenize_msa(
    ...
    for token_idx, center_atom_idx in enumerate(token_center_atom_idxs):
        ...
        all_asym_id_int = atom_array.asym_id_int
        atom_asym_id_int = atom_array[center_atom_idx].asym_id_int
        this_asym_res_id = atom_array.res_id[all_asym_id_int == atom_asym_id_int]
        atom_res_id_min = this_asym_res_id.min()
        
        if (
            res_id := (
                atom_array[center_atom_idx].asym_id_int,
                atom_array[center_atom_idx].res_id - atom_res_id_min + 1,
            )
        ) in res_id_2_msa_idx:
