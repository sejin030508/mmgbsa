...
def centre_random_augmentation(
    ...
    # Generate (rot, trans) pairs.
    # Default (per_frame=False): one (R, t) per N_sample broadcast across batch_size_shape
    # (i.e., shared across frames). Single-frame pretrain needs independent (R, t)
    # for every (frame, sample) pair → per_frame=True.
    batch_size_shape = x_input_coords.shape[:-3]
    if per_frame:
        # N_augment = prod(batch_size_shape) * N_sample → reshape into [*batch, N_sample, 3, 3]
        N_augment = int(np.prod(batch_size_shape)) * N_sample if len(batch_size_shape) > 0 else N_sample
        rot_matrix_random = (
            uniform_random_rotation(N_sample=N_augment)
            .to(device)
            .reshape(*batch_size_shape, N_sample, 3, 3)
        ).detach()  # [*batch_size_shape, N_sample, 3, 3]
        trans_random = s_trans * torch.randn(
            size=(*batch_size_shape, N_sample, 3), device=device
        )  # [*batch_size_shape, N_sample, 3]
        rot_matrix_random = expand_at_dim(
            rot_matrix_random, dim=-3, n=N_atom
        )  # [*batch_size_shape, N_sample, N_atom, 3, 3]
    else:
        rot_matrix_random = (
            uniform_random_rotation(N_sample=N_sample)
            .to(device)
            .reshape(N_sample, 3, 3)
        ).detach()  # [N_sample, 3, 3]
        trans_random = s_trans * torch.randn(size=(1, N_sample, 3)).to(
            device
        )  # [1, N_sample, 3]
        for n_dim in batch_size_shape[::-1]:
            rot_matrix_random = expand_at_dim(rot_matrix_random, dim=0, n=n_dim)
        rot_matrix_random = expand_at_dim(rot_matrix_random, dim=-3, n=N_atom)  # [..., N_sample, N_atom, 3, 3]
    x_augment_coords = (
        rot_vec_mul(
            r=rot_matrix_random, t=x_input_coords
        )
        + trans_random[..., None, :]
    )  # [*batch_size_shape, N_sample, N_atom, 3]
