...
class AttentionTime(nn.Module):
    ...
    def __init__(
        ...
        """
        super(AttentionTime, self).__init__()
        ...
        # Line 6-11
        self.attention_method = "global_attention_with_bias"
        self.attention = Attention(
            ...
            gating=True,
            q_linear_bias=True,
            local_attention_method=self.attention_method,
            use_rotary_embeddings=False,
            use_linear_bias=True,
            first_frame_weight=False,
    ...
    def standard_multihead_attention(
        ...
        a = self.attention(q_x=a, kv_x=a, attn_bias=None, inplace_safe=inplace_safe, time_interval=time_interval)
    ...
    def forward(
        ...
        a = self.standard_multihead_attention(a, s, inplace_safe=inplace_safe, time_interval=time_interval)
        a = a.transpose(0, -2)
...
class DiffusionTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        if time_attn:
            self.attention_time = AttentionTime(
                has_s=True, n_heads=n_heads, c_a=c_a, c_s=c_s, c_z=c_z, biasinit=biasinit,
                causal_mask=causal_mask, beta=beta,
            )
    ...
    def forward(
        ...
        if self.time_attn:
            input_a = attn_out
            attn_out_1 = self.attention_time(
                a=input_a,
                s=s,
                inplace_safe=inplace_safe,
                time_interval=time_interval,
            )
            # print(attn_out_1.abs().mean(-1).mean(-1).mean(-1))
            if inplace_safe:
                attn_out_1 += input_a
            else:
                attn_out_1 = input_a + attn_out_1
        else:
            attn_out_1 = attn_out
        ...
        ff_out = self.conditioned_transition_block(a=attn_out_1, s=s)
        out_a = ff_out + attn_out_1
    ...
    def __init__(
        ...
        for _ in range(n_blocks):
            block = DiffusionTransformerBlock(
                n_heads=n_heads, c_a=c_a, c_s=c_s, c_z=c_z, time_attn=time_attn,
                causal_mask=causal_mask, beta=beta,
            )
...
class AtomAttentionEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.condition_embed = nn.Embedding(2, c_atom)
    ...
    def linear_init(
        ...
        """
        nn.init.zeros_(self.condition_embed.weight[0])
    ...
    def forward(
        ...
        t_hat_noise_level: torch.Tensor = None,
        ...
        if t_hat_noise_level is not None:
            condition = t_hat_noise_level < 1e-3 # t_hat_noise_level [N_frame, N_sample]
            condition[1:-1] = False
            q_l = q_l + self.condition_embed(condition.int()).unsqueeze(-2)
        ...
        q_l = self.atom_transformer(
            q_l, c_l, p_lm, chunk_size=chunk_size, time_interval=input_feature_dict["time_interval"]
        )  # [..., (N_sample), N_atom, c_atom]
...
class AtomAttentionDecoder(nn.Module):
    ...
    def forward(
        ...
        q = self.atom_transformer(
            q, c_skip, p_skip, inplace_safe=inplace_safe, chunk_size=chunk_size, time_interval=input_feature_dict["time_interval"]
        )
