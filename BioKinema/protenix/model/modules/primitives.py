...
def _attention(
    ...
    if first_frame_weight:
        # my modified part: increase first frame (key) weight by adding 0.05 probability mass
        # We shrink the original distribution to 0.90 and add 0.10 to the first key.
        # This ensures sum(attn_weights, dim=-1) is still 1.0.
        attn_weights = attn_weights * 0.9
        attn_weights[..., 0] = attn_weights[..., 0] + 0.1
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        if use_rotary_embeddings:
            self.rot_emb = RotaryEmbedding(dim=self.c_hidden)
        ...
        if use_linear_bias:
            ...
            # 使用 nn.Parameter 包装 slopes，使其可训练
            self.slopes_param = nn.Parameter(self.slopes)
            # 使用 register_buffer 保存位置矩阵 (不参与更新，但随模型保存)
    ...
    def forward(
        ...
        inplace_safe: bool = False,
        chunk_size: Optional[int] = None,
        time_interval: Optional[float] = 1.0,
    ) -> torch.Tensor:
        """
        ...
        if attn_bias is not None:
            # print("attn_bias.shape, q.shape", attn_bias.shape, q.shape)
        ...
        if trunked_attn_bias is not None:
            ...
            # print("trunked_attn_bias.shape, q.shape", trunked_attn_bias.shape, q.shape)
        ...
        if n_queries and n_keys:
            assert not self.use_linear_bias, "Linear bias is not supported with local attention"
            ...
            if self.use_linear_bias:
                # Temporal bias B_ij = -λ|t_i-t_j|^β (β=self.beta; λ=|slopes_param|, ALiBi-init).
                # clamp floors the base so the power is defined at zero separation.
                curr_rel_pos = self.relative_position[:, :tgt_len, :src_len]
                alibi_bias_computed = self.slopes_param.unsqueeze(1).unsqueeze(1) * curr_rel_pos
                alibi_bias_computed = (
                    torch.clamp(-alibi_bias_computed, min=0.000001) ** self.beta
                )
                alibi_bias_computed = -alibi_bias_computed
                if attn_bias is not None:
                    attn_bias = attn_bias + alibi_bias_computed.unsqueeze(0).to(q) * time_interval
                else:
                    attn_bias = alibi_bias_computed.unsqueeze(0).to(q) * time_interval
            
            if self.causal_mask:
                # Create a mask of ones
                temp_mask = torch.ones(tgt_len, src_len, device=q.device, dtype=torch.bool)
            
                # diagonal=1 means the diagonal itself is NOT masked (kept as False in the triu result),
                # so we can attend to the current position.
                future_mask = torch.triu(temp_mask, diagonal=1)
                causal_bias = torch.zeros(tgt_len, src_len, device=q.device, dtype=q.dtype)
                causal_bias = causal_bias.masked_fill(future_mask, -inf)[None, None, :, :]

                # Add to existing attn_bias
                if attn_bias is not None:
                    # Broadcast causal_bias to match attn_bias shape if necessary
            ...
            o = _attention(
                ...
                attn_weight_dropout_p=self.attn_weight_dropout_p,
                inplace_safe=inplace_safe,
                first_frame_weight=self.first_frame_weight
            )  # [*, H, Q, C_hidden]
