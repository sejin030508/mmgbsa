...
class DiffusionConditioning(nn.Module):
    ...
    def forward(
        ...
        single_s = single_s[None, None, :, :] + \
            self.linear_no_bias_n(self.layernorm_n(noise_n))[:, :, None, :]  # [N_frame, N_sample, N_tokens, c_s]
...
class DiffusionModule(nn.Module):
    ...
    def f_forward(
        ...
        if not s_inputs.requires_grad:
            s_inputs.requires_grad_(True) 
        if not s_trunk.requires_grad:
            s_trunk.requires_grad_(True) 
        if not z_trunk.requires_grad:
            z_trunk.requires_grad_(True) 
        ...
        s_trunk =s_trunk.unsqueeze(dim=-3).unsqueeze(dim=-4)  # [1, 1, N_token, c_s]
        z_pair = z_pair.unsqueeze(dim=-4).unsqueeze(dim=-5)  # [1, 1, N_token, N_token, c_z]
        ...
        a_token = self.diffusion_transformer(
            ...
            inplace_safe=inplace_safe,
            chunk_size=chunk_size,
            time_interval=input_feature_dict["time_interval"]
        )
