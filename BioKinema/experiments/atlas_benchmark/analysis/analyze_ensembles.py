# Modified from https://github.com/bjing2016/alphaflow/blob/master/scripts/analyze_ensembles.py

import argparse
parser = argparse.ArgumentParser()
parser.add_argument('--pdbdir', type=str, required=True) 
parser.add_argument('--atlas_dir', type=str, default='/cto_studio/xtalpi_lab/zhangjiying/data/mdgen/data/atlas_sims') 
...
def merge_pdbfiles(input, output_file, verbose=True):
    ...
    pdb_lines = []
    if verbose: 
        _iter = tqdm.tqdm(pdb_files, desc='Merging PDBs')
    else:
        _iter = pdb_files
...
def gather_cif_and_save_pdb(
    cif_dir: str,
    pdb_dir: str,
    name="5znj_A",
):
    pdb_dir = Path(pdb_dir)
    # gather all the cif files
    cif_files = []
    for d in ["R1", "R2", "R3"]:
        cif_dir_tmp = Path(f"{cif_dir}_{d}_0/seed_101/predictions")
        if not cif_dir_tmp.exists():
            cif_dir_tmp = Path(f"{cif_dir}_{d}_0/predictions")
        cif_files += list(cif_dir_tmp.glob("*.cif"))
        
    cif_files = [x for x in cif_files if not str(x).endswith("f0.cif")]
    import random
    random.seed(66)
    random.shuffle(cif_files)
    cif_files = cif_files[:300]
    
    chain_id = name.split("_")[1]
    merged_pdb = Path(pdb_dir) / f"{name}.pdb"
    # if merged_pdb.exists():
    #     print(f">>> {merged_pdb} already exists.")
    #     # return merged_pdb

    # load cifs and save as temp pdbs
    with tempfile.TemporaryDirectory() as tmpdirname:
        for cif_file in cif_files:
            cif_file = Path(cif_file)
            pdb_file = Path(tmpdirname) / (cif_file.stem + ".pdb")
            pdb_structure = load_structure(cif_file)
            # print(pdb_structure[10])
            pdb_structure.set_annotation("chain_id", [chain_id] * len(pdb_structure))
            save_structure(pdb_file, pdb_structure)
            # print(f"Saved pdb file: {pdb_file}")
        # merge all the pdbs into one
        merge_pdbfiles(tmpdirname, merged_pdb, verbose=True)
    return merged_pdb
...
def main(name):
    ...
    try:
        ...
        p = gather_cif_and_save_pdb(
            cif_dir=f'{args.pdbdir}/{name}',
            pdb_dir=f'{args.pdbdir}/pdb',
            name=name,
        )
        aftraj_aa = mdtraj.load(p)
...
if args.pdb_id:
    ...
    pdb_id = [nam for nam in os.listdir(args.pdbdir) if os.path.isdir(f'{args.pdbdir}/{nam}/seed_101/predictions')]
    if len(pdb_id) == 0:
        pdb_id = [nam for nam in os.listdir(args.pdbdir) if os.path.isdir(f'{args.pdbdir}/{nam}/predictions')]
    pdb_id = ["_".join(nam.split("_")[:2]) for nam in pdb_id]
    pdb_id = sorted(list(set(pdb_id)))
...
if args.num_workers > 1:
    ...
    __map__ = map
out_raw = dict(tqdm.tqdm(__map__(main, pdb_id), total=len(pdb_id)))
if args.num_workers > 1:
    p.__exit__(None, None, None)

out = {k: v for k, v in out_raw.items() if v is not None}
print(f"Totally {len(out)} / {len(out_raw)} structures analyzed.")

with open(f"{args.pdbdir}/out.pkl", 'wb') as f:
    f.write(pickle.dumps(out))

print(f">> Saved results to {args.pdbdir}/out.pkl. Exit.")
