...
def load_annotation_mapping(input_csv):
...
def gen_a_bioassembly_data(
    ...
    """
    # if distillation:
    # assert dataset in ["Atlas", "OpenMM", "WeightedPDB"], f"Unsupported dataset: {dataset}"
    kwargs = {}
    if dataset == "OpenMM":
        assert annotation_mapping is not None, "Annotation mapping is required for OpenMM dataset"
        kwargs = annotation_mapping[str(mmcif)] 
...
def run_gen_data(
    ...
    elif input_path.suffix == ".csv":   # annotation mapping
        annotation_mapping = load_annotation_mapping(input_path)
        mmcif_list = list(annotation_mapping.keys())
